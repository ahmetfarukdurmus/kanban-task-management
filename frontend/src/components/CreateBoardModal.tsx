...
export default function CreateBoardModal({
  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box max-w-3xl xl:max-w-4xl w-full p-0 flex flex-col max-h-[88vh] overflow-hidden">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Çalışma alanınız için yeni bir kanban panosu yapılandırın</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="p-6 overflow-y-auto pr-3 flex-1 space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {/* Sol Sütun: Organizasyon Seçimi, Board Adı, Açıklama */}
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    maxLength={500}
                    placeholder="Bu board ne amaçla kullanılıyor? Süreç detaylarını ve hedefleri belirtebilirsiniz..."
              ...
              {/* Sağ Sütun: İş Akışı / Görev Tipi (Workflow), Varsayılan Şablon, Şablon Kolon Önizleme Kutusu */}
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/90 space-y-2.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap pt-2.5 border-t border-slate-200/70">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1 block w-full mb-1 sm:inline-block sm:w-auto sm:mb-0">
                    ...
                    {selectedTaskType && selectedTaskType.columns && selectedTaskType.columns.length > 0 ? (
                      selectedTaskType.columns
                        ...
                        .map((col, idx) => (
                          <span
                            key={col.id || idx}
                            className="inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-md border shadow-2xs"
                            style={{
                      ...
                      selectedTemplate.columns.map((colTitle, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center text-[11px] font-semibold bg-white text-slate-700 px-2.5 py-1 rounded-md border border-slate-200 shadow-2xs"
                        >
                          {idx + 1}. {colTitle}
                        </span>
                      ))
